...
class UserController {
    ...
    async deleteUser(data){
        this.#verifyType(data.callingUser.type, USER_TYPES.ADMIN);
        const daluser =  await this.#userRepo.getUser(data.username);
    ...
    async setUserAsTA(data){
        this.#verifyType(data.callingUser.type, USER_TYPES.LECTURER)
        const daluser =  await this.#userRepo.getUser(data.username);
        if (daluser.userType === USER_TYPES.ADMIN){
            throw new EMSError("can't remove user from being an admin");
        }
        this.#userRepo.changeType(data.username, USER_TYPES.TA);
    }

    async setUserAsLecturer(data){
        this.#verifyType(data.callingUser.type, USER_TYPES.LECTURER)
        const daluser =  await this.#userRepo.getUser(data.username);
        if (daluser.userType === USER_TYPES.ADMIN){
            throw new EMSError("can't remove user from being an admin");
        }
        this.#userRepo.changeType(data.username, USER_TYPES.LECTURER);
    }
